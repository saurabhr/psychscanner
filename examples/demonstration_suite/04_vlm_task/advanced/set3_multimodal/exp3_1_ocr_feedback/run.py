"""Exp 3.1 — BPT-10: OCR-VLM × With Feedback.
...
Task  : Behavioral Profiling Task — 10 sequences × 10 study items + 1 probe
Memory: chain_type=task, Convo, no trimming
Feedback: BPTFeedback (correct/incorrect + correct answer after each probe)
"""
...
if CSV_PATH.exists():
    ...
    task_dict = build_task(ocr=True, feedback=True)
...
if "resp_recalled_number" not in probe_df.columns:
    ...
    # Figure 1: accuracy by serial position
    ...
    ax.set_title(f"Exp 3.1 — Lost-in-the-Middle\nOCR-VLM + Feedback | model: {MODEL}", fontsize=10)
    ...
    plt.close()
    print("Saved figures/serial_position_curve.png")

    # Figure 2: accuracy by probe target modality (Attentional Residue)
    if "probe_target_modality" in scored_pd.columns:
        ...
        plt.close()
        print("Saved figures/modality_switch_effect.png")

print(f"\nExp 3.1 complete.  {len(df)} total rows.")
