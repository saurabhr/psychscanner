"""Exp 4.3 — VISER Counting: Non-OCR × With VISER.

Run from the psychscanner project root:
    conda activate psyscan
    python examples/demonstration_suite/04_vlm_task/advanced/set4_viser/exp4_3_nonocr_viser/run.py

Model : nvidia/nemotron-nano-12b-v2-vl:free  (OpenRouter)
Task  : Counting task — 10 scenes × 1 study + 1 probe
VISER : Yes — grid description in text + sequential prompt
"""
...
if CSV_PATH.exists():
    ...
    task_dict = build_task(ocr=False, viser=True)
...
if "resp_recalled_number" not in probe_df.columns:
    ...
    ax.bar([0], [accuracy], color="tomato", alpha=0.8)
    ax.set_xticks([0])
    ax.set_xticklabels(["Non-OCR + VISER"])
    ax.set_ylabel("Accuracy")
    ax.set_title(f"Exp 4.3 — Counting Accuracy\nModel: {MODEL}", fontsize=10)
    ax.set_ylim(0, 1)
...
print(f"\nExp 4.3 complete.  {len(df)} total rows.")
