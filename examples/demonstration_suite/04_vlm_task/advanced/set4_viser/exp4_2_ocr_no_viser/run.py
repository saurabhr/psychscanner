"""Exp 4.2 — VISER Counting: OCR-VLM × No VISER.

Run from the psychscanner project root:
    conda activate psyscan
    python examples/demonstration_suite/04_vlm_task/advanced/set4_viser/exp4_2_ocr_no_viser/run.py
...
if CSV_PATH.exists():
    ...
    task_dict = build_task(ocr=True, viser=False)
...
if "resp_recalled_number" not in probe_df.columns:
    ...
    ax.set_title(f"Exp 4.2 — Counting Accuracy\nModel: {MODEL}", fontsize=10)
    ax.set_ylim(0, 1)
...
print(f"\nExp 4.2 complete.  {len(df)} total rows.")
