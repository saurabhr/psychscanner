"""Exp 4.4 — VISER Counting: Non-OCR × No VISER.

Run from the psychscanner project root:
    conda activate psyscan
    python examples/demonstration_suite/04_vlm_task/advanced/set4_viser/exp4_4_nonocr_no_viser/run.py
...
RUN_LABEL = "exp4_4_nonocr_no_viser"
...
if CSV_PATH.exists():
    ...
    task_dict = build_task(ocr=False, viser=False)
...
if "resp_recalled_number" not in probe_df.columns:
    ...
    ax.set_title(f"Exp 4.4 — Counting Accuracy\nModel: {MODEL}", fontsize=10)
    ax.set_ylim(0, 1)
...
print(f"\nExp 4.4 complete.  {len(df)} total rows.")
