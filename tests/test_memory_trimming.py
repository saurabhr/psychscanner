...
def test_summary_k_does_not_silently_drop_overflow_below_threshold():
    """Regression: summary_k=3 with memory_k=4 never lines up with this
    graph's 2-messages-per-turn growth (overflow batches land at 2, not a
    multiple of 3), so the pre-fix code deleted every overflow batch
    without ever reaching the threshold -- silently losing all trimmed
    history, permanently. Turn 0's content must still be recoverable from
    the summary once enough turns have accumulated overflow past the
    threshold.
    """
