"""Exp 3.3 — BPT-10: non-OCR-VLM × With Feedback.

Text-only model; image-slot items delivered as "[IMAGE TEXT] WORD → NUMBER".
...
Model : mistralai/mistral-nemo-12b-instruct:free  (via OpenRouter)
"""
...
RUN_LABEL = "exp3_3_nonocr_feedback"
...
if CSV_PATH.exists():
    ...
    task_dict = build_task(ocr=False, feedback=True)
...
if "resp_recalled_number" not in probe_df.columns:
    ...
    ax.set_title(
        f"Exp 3.3 — Lost-in-the-Middle\nnon-OCR + Feedback | model: {MODEL}", fontsize=10
    )
...
print(f"\nExp 3.3 complete.  {len(df)} total rows.")
